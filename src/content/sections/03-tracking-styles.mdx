...
- **Push + Graph**: TanStack Store (`Derived` with explicit `deps`), Verge (`Derived` with pipeline), Atoms (implicit tracking via `watch()`)
...
- **Atoms**: `context.watch(OtherAtom())` — tracked at `watch()` call time (push propagation)
