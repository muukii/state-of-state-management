...
**Every state management system needs derived state.** Whether store-centric or atomic/graph-centric, applications inevitably need to compute values from other values — filtered lists, aggregated counts, combined UI states.
...
This technique appears in several runtime shapes:

- **Cached store/atom graph**: Verge (`Derived` with pipeline), Atoms (implicit tracking via `watch()`)
- **Read-time capture graph**: Jotai (`get()`), TanStack Store (`createStore(() => other.state)`), Riverpod (`ref.watch()`), swift-state-graph (ThreadLocal), Compose `derivedStateOf`
...
- **Atoms**: `context.watch(OtherAtom())` — tracked at `watch()` call time
- **Riverpod**: `ref.watch(provider)` — provider dependency captured during build
- **TanStack Store**: `createStore(() => otherStore.state * 2)` — store reads are tracked by the reactive runtime
...
The developer declares dependencies via a pipeline or composition API. The graph is still managed by the runtime, but the developer controls what goes in.
